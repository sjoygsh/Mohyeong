...
(function () {
  ...
  function popular(page) {
    return getHtml(BASE + '/manga/page/' + page + '/?m_orderby=views').then(function (html) {
      return { mangas: parseList(html), has_next_page: hasNext(html, page) };
    });
  }
  function latest(page) {
    return getHtml(BASE + '/manga/page/' + page + '/?m_orderby=latest').then(function (html) {
      return { mangas: parseList(html), has_next_page: hasNext(html, page) };
    });
  }
